 ...
 */
function loadMap(pos, node, link) {
  ...
  u.on("tick", function() {
    ...
    d.attr({
      ...
      transform: function(t) {
        ...
        return "rotate(" + 360 * Math.atan(r / e) / (2 * Math.PI) + "," + (t.target.x + t.source.x) / 2 + "," + (t.target.y + t.source.y) / 2 + ")"
      }
    });
  });
  u.charge(-1300).linkDistance(200).nodes(node).links(link).size([w, h]).alpha(.1);
  o.append("svg:defs").append("svg:marker").attr("id", "arrow").attr("viewBox", "0 -5 10 10")
      .attr("refX", 6).attr("markerWidth", 5).attr("markerHeight", 5).attr("orient", "auto")
      .append("svg:path").attr("d", "M0,-5L10,0L0,5").attr("fill", "#6ac6ff");
  s.selectAll("line.link").data(link).enter().append("line").attr("class", "link");

  var d = s.selectAll("link.desc").data(link).enter().append("text")
      .attr("class", "desc").text(function(t) {
        return t.desc
      }),
      f = (u.drag().on("dragstart", function(t) {
        t.fixed = !0
      }),
          s.selectAll("g.node").data(node)),
